...
実際にアプリケーションを操作していただけるデモ環境を用意しています。

デモ環境へのアクセスにはBasic認証が必要です。
また、アプリケーションへのログインにはテスト用アカウントを使用します。

Basic認証およびテスト用アカウントの認証情報は、応募書類に記載しています。

### システム管理者で確認できる機能

システム管理者としてログインすると、以下の機能を操作できます。
...
1. 上記URLへアクセス
2. 応募書類に記載しているBasic認証のユーザー名・パスワードを入力
3. 応募書類に記載しているテスト用アカウントでログイン
4. ダッシュボードから各機能を操作

システム管理者でログイン後、一般ユーザーを作成することで、一般ユーザーとシステム管理者の権限による操作の違いも確認できます。
...
同一の初期データで `db:seed` を複数回実行しても、重複して登録されないようにしています。
